...
export const RULE_SLUG_TO_ID = Object.freeze({
    NO_MOCKING: RULE_IDS.NO_MOCKING,
    NO_HARDCODE: RULE_IDS.NO_HARDCODE,
    NO_SILENT_FALLBACKS: RULE_IDS.NO_SILENT_FALLBACKS,
    NO_INTERNAL_CACHING: RULE_IDS.NO_INTERNAL_CACHING,
    NO_EMOJI: RULE_IDS.NO_EMOJI,
    NO_STRING: RULE_IDS.NO_STRING,
    NO_CONSOLE: RULE_IDS.NO_CONSOLE,
    BINARY_AST_ONLY: RULE_IDS.BINARY_AST_ONLY,
    STRICT_COMMENT_STYLE: RULE_IDS.STRICT_COMMENT_STYLE,
    MUST_HANDLE_ERRORS: RULE_IDS.MUST_HANDLE_ERRORS
});
...
export function resolveRuleId(slug) {
    if (typeof slug === 'string' && RULE_SLUG_TO_ID[slug]) {
        return RULE_SLUG_TO_ID[slug];
    }
    return null;
}

export function coerceRuleId(value) {
    ...
    if (typeof value === 'string') {
        ...
        if (RULE_SLUG_TO_ID[value]) {
            return RULE_SLUG_TO_ID[value];
        }
    }
